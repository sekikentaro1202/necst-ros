...
class telescope(object):
    ...
    def move_otf_mode(self):
        ...
        lamda = float(self.tel_value['obs_wavelength'])/1000000.
        ...
        ### 
            if coord_sys_on == 'EQUATORIAL':
                con.radec_move(lam_on  ,bet_on , on_coord, off_x = start_x + lam_del, off_y = start_y + bet_del, offcoord = coord_sys_del, dcos = dcos_kosma)
                ...
                con.galactic_move(lam_on ,bet_on , off_x = start_x + lam_del, off_y = start_y + bet_del, offcoord = coord_sys_del, dcos = dcos_kosma)
                ...
                con.azel_move(lam_on + (lam_del+start_x)/3600., bet_on + (bet_del+start_y)/3600.)
    ...
    ###otf scan start(1 line)
        if duration != 0 and self.pre_otf_position_flag == 1:
            ...
            start_on = con.otf_scan(lam_on, bet_on, coord_sys_on, lam_vel, bet_vel, 1, duration, 3, 0, _list, lam_del, bet_del, coord_sys_del, dcos_kosma, 'hosei_230.txt', lamda)  
    ...
    def move_telescope(self):
        ...
        if coord_on == 'horizontal':
            ...
            con.azel_move(obs_lam_on+lam_off*3600.+lam_del*3600, obs_bet_on+bet_off*3600.+bet_del*3600.)
            to_print = ['horizon##',obs_lam_on*3600.+lam_off+lam_del, obs_bet_on*3600.+bet_off+bet_del]
            self.log.info(" ".join([str(ele) for ele in to_print]))
        elif coord_on.lower() == 'galactic':
            con.galactic_move(lam_on_off, bet_on_off, off_x = lam_del, off_y = bet_del, offcoord = coord_map_offsets, lamda = obs_wavelength)
            ...
            con.radec_move(lam_on_off, bet_on_off, coord_on,off_x = lam_del, off_y = bet_del, offcoord = coord_map_offsets, lamda = obs_wavelength)
            # hack from ronan
        ...
        while not st.read_status()[10]['tracking']:#一応3回目
            time.sleep(0.001)
            continue
        self.log.info('tracking : {0}'.format(st.read_status()[10]['tracking']))
        self.tel_on_track = 'Y'
